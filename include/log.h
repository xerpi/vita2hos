...
#define IF_VERBOSE                                                                                 \
    if (VERBOSE_LOGGING) ///< Place before calling @c LOG to only show when verbose output is
                         ///< enabled
#define __FILENAME__ (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)
// #define LOG(...)        {printf("%s:%d: ", __FILENAME__, __LINE__); printf(__VA_ARGS__);
// puts("");}  ///< Write a log entry
