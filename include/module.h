...
#define NUM_EXPORTED_LIBS                                                                 \
	(((uintptr_t)__stop_exported_libraries - (uintptr_t)__start_exported_libraries) / \
	 sizeof(library_t))
#define NUM_FUNCS(lib) \
	(((uintptr_t)lib->func_table_stop - (uintptr_t)lib->func_table_start) / sizeof(void *))
